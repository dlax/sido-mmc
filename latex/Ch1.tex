...
Le lecteur pourra se référer aux traités classiques (voir \tmp{bibliographie, (1) à (10)}).
...
\begin{equation}
    ...
    \label{eq:Ch01-001}
\end{equation}
où $\frac{\ud}{\ud t}$ est la dérivée «~particulaire~», c'est à dire la dérivée obtenue en suivant le domaine $D$ dans son mouvement (voir par exemple \tmp{[2]}).
...
\end{equation*}
On trouvera des démonstrations plus détaillées et plus rigoureuses en \tmp{[1], [2], [3], [5]....}
...
\end{wrapfigure}
On peut alors montrer (voir \tmp{[2]}) que l'équation locale~\eqref{eq:Ch01-008} doit être complétée par une «~équation aux discontinuités~»:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:Ch01-025}
\end{equation}
On peut s'assurer (voir \tmp{[I],[5]}) que c'est une interprétation justifiée dans la mesure où elle généralise la puissance virtuelle des efforts intérieurs introduite en mécanique rationnelle pour un système de solides rigides.
