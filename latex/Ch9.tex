...
\subsection{Théorème de l'énergie potentielle} \label{ssec:Ch09-1.2}
...
\begin{proof}
    ...
    Si $S_u$ existe (plus précisément, si $S_u$ est de mesure nulle \tmp{n'est-ce pas un peu paradoxal ?}), les conditions aux limites en déplacement permettent de montrer que $\vec{\omega} = \vec{\alpha} = 0$ 
...
\end{proof}
\subsection{Théorème de l'énergie complémentaire} \label{ssec:Ch09-1.3}
...
On peut d'ailleurs obtenir directement ce résultat par une approche énergétique.
...
\begin{multicols}{2}
    ...
    \]
\end{multicols}\tmp{c'est un peu bizarre ce $f_i$ isolé}
\[
f_i = 0
\]
\begin{equation}
...
\begin{align}
    ...
    \label{eq:Ch09-037}
\end{align}
Pour calculer $T_u^d$ et $T_f^d$ il faut expliciter la décomposition~\eqref{eq:Ch06-008} pour le problème \eqref{eq:Ch09-033}\tmp{bizarre...}: 
\begin{equation*}
    \begin{aligned}
...
Un CCA $\tilde{u}_i$ doit uniquement vérifier les conditions aux limites cinématiques: 
\begin{equation}
    \begin{aligned}
...
Les conditions \eqref{eq:Ch09-038} et \eqref{eq:Ch09-039} entraînent la solution:
\begin{equation}
...
\subsection{Théorème de réciprocité} \label{ssec:Ch09-2.1}
...
\subsection{Théorème de Castigliano} \label{ssec:Ch09-2.2}
...
L'utilisation de ce théorème et du théorème de réciprocité est basée sur le fait qu'en introduisant comme chargement 2, des chargements fictifs, il est possible de calculer certains déplacements ou déformations.
...
\subsection{Principe} \label{ssec:Ch09-3.1}
...
\begin{figure}[htp]
\centering
\subfloat[Elément triangulaire avec une fonction linéaire]{
\includegraphics{../images/T1_Ch09-02}
}\quad
\subfloat[Elément rectangulaire avec fonction bilinéaire]{
\includegraphics{../images/T1_Ch09-02}
}\quad
\subfloat[Elément triangulaire avec fonction quadratique]{
\includegraphics{../images/T1_Ch09-02}
}
\end{figure}
...
\begin{description}
    \item 1\fup{re} méthode --- On discrétise $\Sigma$ et on maximise la fonctionnelle $h(\hat{\varphi})$ sur l'espace des fonctions $\hat{\varphi}$ nulles sur le bord; 
    \item 2\fup{e} méthode --- On discrétise $\Sigma$ et on minimise la fonctionnelle $h(\hat{\psi})$ sur l'espace de toutes les fonctions $\hat{\psi}$.
\end{description}

\subsection{Application} \label{ssec:Ch09-3.2}
